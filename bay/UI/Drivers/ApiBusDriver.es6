...
Core.UI.Drivers.ApiBusDriver = class
{
	...
	static buildPostDataRecursive(res, data, key)
	{
		...
		else if (data instanceof Runtime.Dict)
		{
			data.each(
				(i, item) => {
					Core.UI.Drivers.ApiBusDriver.buildPostDataRecursive(
						res, 
						item, 
						(key == "") ? i : key + "[" + i + "]"
					);
	...
	sendApi(class_name, method_name, data, f)
	{
		...
		/* Build pos data */
		data = Core.UI.Drivers.ApiBusDriver.buildPostData(data);
		...
		/* Add CSRF Token */
		/*post_data.append("csrf_token", Core.UI.Drivers.ApiBusDriver.GetCookie("csrf_token"));*/
		
		/* Send AJAX Request */
		var xhr = new XMLHttpRequest();
		xhr.open('POST', '/api/' + class_name + '/' + method_name + '/', true);
		xhr.send(post_data);
